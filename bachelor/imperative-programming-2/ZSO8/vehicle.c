...
enum keuze {AUTO, VLIEGTUIG};
typedef enum keuze keuze;
...
void readVehicle(struct vehicle* voertuig) {
	...
	if(keuze == 1)
		voertuig->choice = AUTO;
	else
		voertuig->choice = VLIEGTUIG;
	if(keuze == 1) {
...
void printVehicle(struct vehicle* voertuig) {
	...
	else if(voertuig->choice == VLIEGTUIG) {
