...
function clockTick() {
	...
	for(var i = 0; i < activeGames.length; ++i) {
		...
		var timeDiffSeconds = (currentTime - game.timeout) / 1000
		var room = game.room
...
function eventStartGame(connection, session, data) {
	if(users[session.id].room != null) { // Requester must be IN a room
		var room = rooms[users[session.id].room]
		...
		if(users.length < 2) { // Must have at least 2 participants in the room
			for(var key in users)
				sendEvent(users[key].id, users[key].connection, "chat-to-room", {
					author: {
						id: 0,
